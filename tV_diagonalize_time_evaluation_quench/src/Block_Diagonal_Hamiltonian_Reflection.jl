...
"""
function Block_Diagonal_Hamiltonian_Reflection(basis::AbstractSzbasis, Cycles:: Array{Int64,2}, CycleSize:: Vector{Int64}, NumOfCycles::Int64, t::Float64, V::Float64, q::Int64, P::Int64)
    if abs(P)!=1
        warn("abs(P)!=1, ", "  quit")
        quit()
    end
                    ...
                    if bra[site1] > 0
                        ...
                        if ket in basis
                            ...
                            for (k1, CycleId1) in enumerate(InvolvedCycles)
                                ...
                                if kIdcy>0
                                    phase=exp_q[kIdcy]
                                    Hq[k1, i]+=-t* phase*sqrt(abs(CycleSize[CycleId])/abs(CycleSize[CycleId1]))/2.0
                                    Hq[i, k1]+=conj(-t* phase*sqrt(abs(CycleSize[CycleId])/abs(CycleSize[CycleId1])))/2.0
