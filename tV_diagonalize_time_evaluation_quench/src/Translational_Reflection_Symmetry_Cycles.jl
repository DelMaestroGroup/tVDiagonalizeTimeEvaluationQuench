"""
Create a list of occupation basis for each translational and reflection symmetry cycle for fermionic 1D chains.
"""
    ...
    for (i, bra) in enumerate(basis)
        ...
        for j =0: 1
            if Status[i_next]
                ...
                if j==1
                    CReflection[NumOfCycles]=true
                    CReflection[NumOfCycles-1]= true
                end 
            end
            i_next=serial_num(basis,reverse(bra))
        end
    end
    Cycles, CycleSize, NumOfCycles, CReflection
end
