   ...
   for P=1:-2:-1*p1 
      ...
      for i_HqRank =1: HqRank
         for j_HqRank =1: HqRank
            CycleSizeijloop =CycleSize[j_HqRank]
            factor= VV[j_HqRank, i_HqRank]/sqrt(CycleSizeijloop)
            EigenVector[Cycles[j_HqRank ,1: CycleSizeijloop]]= factor*(1.0+0.0im) #exp_q[1: CycleSizeijloop]* factor
         end
...
#     end
#
#     s3_particle = particle_entropy_Ts(basis, Asize, wft0, site_max)
...
if abs(time_range[1]) < 1.0E-12
    ...
    if c[:obdm] && Asize == 1
        s_particle,obdm[:,1] = particle_entropy_Ts(basis, Asize, Ψt[:,1], site_max,c[:obdm])
    else
        s_particle = particle_entropy_Ts(basis, Asize, Ψt[:,1], site_max,c[:obdm])
    end
    ...
    if c[:spatial]
        s_spatial = spatial_entropy(basis, ℓsize, Ψt[:,1])
...
for time in time_range[time_start:end] 
    ...
    if c[:obdm] && Asize == 1
        s_particle,obdm[:,it] = particle_entropy_Ts(basis, Asize, Ψt[:,it], site_max,c[:obdm])
    else
        s_particle = particle_entropy_Ts(basis, Asize, Ψt[:,it], site_max,c[:obdm])
    end
