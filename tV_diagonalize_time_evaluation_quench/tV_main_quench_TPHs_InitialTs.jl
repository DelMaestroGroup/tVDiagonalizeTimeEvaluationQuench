...
if abs(time_range[1]) < 1.0E-12
    ...
    if c[:obdm] && Asize == 1
        s_particle,obdm[:,1] = particle_entropy_Ts(basis, Asize, Ψ, site_max,c[:obdm])
    else
        s_particle = particle_entropy_Ts(basis, Asize, Ψ, site_max,c[:obdm])
    end
